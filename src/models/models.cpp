              ...
              const std::string &description, int total)
{
    db()->execSqlSync(
        ...
        "VALUES (?, ?, ?, ?, ?)",
        name, author, description, total, total);
...
void Book::remove(int id)
{
    auto check = db()->execSqlSync(
        ...
        std::to_string(id));
    if (!check.empty())
        throw std::runtime_error("该书尚有借出记录，无法删除");
                  ...
                  const std::string &password)
{
    if (password.empty())
    {
        db()->execSqlSync("UPDATE users SET username=? WHERE id=?",
                          username, std::to_string(id));
    }
    else
    {
        auto salt = randomSalt();
...
void User::remove(int id)
{
    auto check = db()->execSqlSync(
        ...
        std::to_string(id));
    if (!check.empty())
        throw std::runtime_error("该用户尚有借出记录，无法删除");
...
void Library::borrowBook(int userId, int bookId)
{
    ...
    if (res.affectedRows() == 0)
        throw std::runtime_error("暂无库存可借");
    ...
    catch (...)
    {
        db()->execSqlSync(
            ...
            std::to_string(bookId));
        throw std::runtime_error("已借阅过此书");
    }
}
...
void Library::returnBook(int userId, int bookId)
{
    auto res = db()->execSqlSync(
        ...
        std::to_string(userId), std::to_string(bookId));
    if (res.affectedRows() == 0)
        throw std::runtime_error("未找到借阅记录");
