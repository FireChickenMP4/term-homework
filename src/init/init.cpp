...
void init(drogon::HttpAppFramework& app) {
    ...
    if (f.good()) {
        ...
        app.addDbClient(drogon::orm::MysqlConfig{
            .host = cfg.dbHost, .port = (unsigned short)cfg.dbPort,
            .databaseName = cfg.dbName, .username = cfg.dbUser,
            .password = cfg.dbPassword, .connectionNumber = 1,
            .name = "default", .isFast = false,
            .characterSet = "utf8mb4", .timeout = 1.0
        });
